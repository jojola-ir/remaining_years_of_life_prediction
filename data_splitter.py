...
def generate_dataset(path_to_images, path_to_output, organ="heart", test=False):
    ...
    if test:
        imagePathInput = join(path_to_images, 'imagesTs/')

        imageSliceOutput = join(path_to_output, 'images/')
        ...
        imagePathInput = join(path_to_images, 'imagesTr/')
        maskPathInput = join(path_to_images, 'labelsTr/')

        imageSliceOutput = join(path_to_output, 'images/')
        maskSliceOutput = join(path_to_output, 'masks/')
...
def random_splitter(src, dest, test_rate, clear):
    ...
    if clear:
        ...
        for root, _, files in os.walk(src):
            if root == join(src, "masks"):
                for f in files:
                    if not f.endswith(".DS_Store"):
                        maskpath = join(root, f)
                        imagepath = join(join(src, "images/"), f)
